    ...
    let
        ...
        upButton =
            if isFirstElement then
                ...
                R.upButton questionId mdl questionMovedUp
        ...
        downButton =
            if isLastElement then
                ...
                R.downButton questionId mdl questionMovedDown
    in
        Card.view
            [ Elevation.e4
            , css "width" "100%"
                ...
                [ Options.div
                    ...
                    [ Options.div
                        ...
                        [ if question.isExpanded then
                            R.questionIsExpanded questionId mdl <| QuestionUpdated questionId Collapse
                          else
                            R.questionIsCollapsed questionId mdl <| QuestionUpdated questionId Expand
                        ]
                    , Options.div
                        [ css "flex-grow" "1"
                        ...
                        , Textfield.render Mdl
                            questionId
                            mdl
                            [ Options.onInput <| QuestionUpdated questionId << TitleUpdated
    ...
    let
        ...
        renderButton ( qType, typeId, menuOptions ) =
            [ Button.render Mdl
                ...
                [ Button.raised
                , Button.colored
                , (if questionTypeToString questionType /= questionTypeToString qType then
                    (css "" "")
                   else
                    Button.disabled
                  )
                , Options.onClick <| QuestionUpdated id <| TypeChanged qType
                , css "width" "25%"
                , css "font-size" "9pt"
                , css "padding" "0pt"
                ]
            ...
            , if
                questionTypeToString qType
                    /= questionTypeToString (SubQuestionContainer [])
                    && questionTypeToString qType
                    == questionTypeToString questionType
              then
                let
                    menuType1 =
                        Maybe.withDefault TextT <| List.head menuOptions

                    menuType2 =
                        Maybe.withDefault TextT <|
                            List.head <|
                                Maybe.withDefault [ TextT ] <|
                                    List.tail menuOptions
                in
              ...
              else if questionTypeToString qType /= questionTypeToString (SubQuestionContainer []) then
                Options.div [ css "margin" "0px", css "padding" "0px", css "width" "10%" ] []
              else
                Options.div [ css "margin" "0px", css "padding" "0px", css "width" "0%" ] []
            ]
    in
        Options.div
            [ css "justify-content" "space-between"
        ...
        <|
            List.concat <|
                List.map renderButton questionButtonTypes
...
renderQuestionSpecificControl id mdl ({ questionType, title } as question) =
    case questionType of
        ...
        SubQuestionContainer questions ->
            let
                controls =
                    List.indexedMap (renderControl id mdl <| List.length questions) questions
            in
                            ...
                            , [ MList.li [] <|
                                    [ MList.content []
                                        [ Button.render Mdl
                                            ...
                                            [ Button.raised
                                            , Button.ripple
                                            , Options.onClick <| SubQuestionAdded id
                                            , Button.colored
                                            ]
...
renderOption questionId mdl placeholder option =
    div []
        [ Textfield.render Mdl
            ...
            mdl
            [ Options.onInput <| QuestionUpdated questionId << MultipleChoiceOptionUpdated option.id
